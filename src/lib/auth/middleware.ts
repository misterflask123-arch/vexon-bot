...
export const authMiddleware = createMiddleware({ type: "function" })
  ...
  .server(async ({ next, context }) => {
    ...
    // Keep the preview bearer token available to server handlers. The token is
    // needed by Better Auth's getAccessToken() API when the preview is inside
    // an iframe and the session cookie is not visible to the server request.
