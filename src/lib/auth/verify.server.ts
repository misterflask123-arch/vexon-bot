...
/** True when a real database is configured server-side. Live on Workers. */
function isDatabaseConfigured(): boolean {
  const fromProcess =
    typeof process !== "undefined" ? process.env.DATABASE_URL : undefined;
  const fromCf = (globalThis as typeof globalThis & { __env__?: { DATABASE_URL?: unknown } })
    .__env__?.DATABASE_URL;
  const raw =
...
export { isAuthConfigured, isAuthConfigured as authConfigured };
...
export async function getSessionUser(
  ...
  let headers: Headers;
  try {
    // TanStack Start exposes the incoming server-function headers through this
    // helper. Reading getRequest() here can return no request on Vercel even
    // while the browser still has a valid Better Auth session.
