...
import { GROK_PROVIDERS, type GrokProvider } from "./providers";
...
/** True when Discord sign-in is enabled (VITE_DISCORD_ENABLED=true or unset with DISCORD_CLIENT_ID set server-side). */
export const discordEnabled = import.meta.env.VITE_DISCORD_ENABLED !== "false";

/** Discord provider entry for the sign-in UI. */
const DISCORD_PROVIDER: GrokProvider = {
  providerId: "discord",
  idp: "discord",
  label: "Discord",
};

/** All available sign-in providers (Grok broker + Discord when enabled). */
export const ALL_PROVIDERS: readonly GrokProvider[] = [
  ...GROK_PROVIDERS,
  ...(discordEnabled ? [DISCORD_PROVIDER] : []),
];
