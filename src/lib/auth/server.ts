...
import { stashDiscordGrant, takeLatestDiscordGrant } from "@/lib/discord/grant";
...
export function isAuthConfigured(): boolean {
  return !isAuthDisabled() && Boolean(discordClientId() && discordClientSecret());
}
...
function discordProviderConfig() {
  return {
    ...
    /**
     * Capture the plaintext user access token here — Better Auth encrypts it
     * before writing `account`. The session create hook copies it onto the
     * session row so listing guilds never calls getAccessToken().
     */
    async getUserInfo(tokens: { accessToken?: string | null }) {
      ...
      if (!id) return null;
      stashDiscordGrant(id, accessToken);
      return {
        ...profile,
        id,
...
function createAuth() {
  ...
  return betterAuth({
    ...
    session: {
      // Keep Discord sessions across browser restarts for 30 days.
      expiresIn: 60 * 60 * 24 * 30,
      updateAge: 60 * 60 * 24 * 7,
      cookieCache: { enabled: true, maxAge: 300 },
      additionalFields: {
        // Never returned to the browser / cookie cache.
        discordAccessToken: {
    ...
    databaseHooks: {
      session: {
        create: {
          before: async () => {
            const grant = takeLatestDiscordGrant();
            if (!grant) return;
            return {
              data: {
                discordAccessToken: grant.accessToken,
