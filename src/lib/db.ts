...
/**
 * Live DATABASE_URL. Cloudflare Workers bind env per-request, so this MUST
 * not be snapshotted at module load — an empty read would pin the isolate to
 * PGLite for its whole lifetime.
 */
function getDatabaseUrl(): string | undefined {
  const fromProcess =
    typeof process !== "undefined" ? process.env.DATABASE_URL : undefined;
  const fromCf = cfEnv()?.DATABASE_URL;
  const raw =
 ...
 *
 * Live on Workers — do not treat this as a constant.
 */
...
export function ensureDbReady(): Promise<void> {
  // Neon: pool is created lazily on first query.
  // Cloudflare Workers: env isn't bound at module evaluation — don't start
  // PGLite here or the isolate would pin to the embedded DB forever.
  if (getDatabaseUrl() || isCloudflareWorker()) return Promise.resolve();
...
if (typeof window === "undefined" && !isCloudflareWorker() && !getDatabaseUrl()) {
