 ...
 *   2. The Discord user id + access token live on that session row (copied there
 *      at Discord sign-in). Listing guilds uses them directly against Discord.
 *   3. Opening a guild mints a **one-shot** HS256 token (≤60s, unique `jti`,
 *      bound to the guild) with the secret shared with the bot.
...
export const NOT_LINKED_MESSAGE =
  "لا يوجد توكن Discord في الجلسة — سجّل الدخول عبر Discord أو اربط حسابك من أعلى الصفحة.";
...
async function botCall<T>(
  ...
  const [{ getSessionDiscordGrant }, { SignJWT }, { randomUUID }] = await Promise.all([
    import("@/lib/discord/oauth.server"),
    import("jose"),
    import("node:crypto"),
  ]);

  const linked = await getSessionDiscordGrant(bearerToken);
 ...
 *      (`GET /users/@me/guilds`, Bearer session access_token).
     *   2. The bot is actually in that guild (GET /stats, another bot guild
     *      endpoint, or the bot token's own /users/@me/guilds). No signed Admin
     *      API on this path.
 *
 * A server appears only when both agree. Opening a guild still uses the
 * signed Admin API.
 */
export const listGuilds = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<GuildSummary[]> => {
    const {
      DiscordLinkError,
      getSessionDiscordGrant,
      fetchUserGuilds,
      isGuildManager,
      discordGuildIconUrl,
      discordGuildIconColor,
    } = await import("@/lib/discord/oauth.server");
    const { fetchBotGuildIds } = await import("@/lib/dashboard/bot-guilds");

    const grant = await getSessionDiscordGrant(context.bearerToken);
    if (!grant) throw new Error(NOT_LINKED_MESSAGE);

    let mine;
    try {
      mine = await fetchUserGuilds(grant.token);
    } catch (error) {
      if (error instanceof DiscordLinkError && error.upstreamStatus === 401) {
        throw new Error("توكن ديسكورد منتهي أو ملغى — سجّل الدخول عبر Discord من جديد.");
      }
      throw error;
    }

    const managed = mine.filter(isGuildManager).slice(0, 200);
     const botIds = await fetchBotGuildIds();
     if (!botIds) {
       throw new Error("تعذر التحقق من سيرفرات البوت — أعد المحاولة لاحقاً.");
     }
     const visible = managed.filter((g) => botIds.has(g.id));

    return visible.map((g) => ({
      id: g.id,
      name: g.name,
      iconUrl: discordGuildIconUrl(g.id, g.icon),
      iconColor: discordGuildIconColor(g.id),
      memberCount: g.approximate_member_count ?? 0,
      locale: "ar",
      staffRoleId: null,
      owner: Boolean(g.owner),
      administrator: !g.owner && isGuildManager(g),
    }));
  });
