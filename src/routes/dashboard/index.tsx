...
import { AlertTriangle, ChevronLeft, Link2, Server, Shield } from "lucide-react";
...
function DashboardHome() {
  ...
  const loadStatus = useCallback(async (): Promise<DiscordLinkStatus | null> => {
    setStatusErr(null);
    ...
    try {
      const result = await Promise.race([
        ...
        new Promise<never>((_, reject) => {
          timer = window.setTimeout(
            () => reject(new Error("انتهت مهلة التحقق من جلسة Discord — أعد المحاولة.")),
            12_000,
          );
        }),
      ]);
      ...
      const message = error instanceof Error ? error.message : "تعذر تحميل حالة Discord.";
      setStatusErr(message);
      return null;
    } finally {
      ...
      <div className="flex min-h-screen flex-col bg-bg">
        ...
        <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-10 sm:px-6">
          ...
          <p className="mt-2 max-w-xl text-sm leading-relaxed text-muted">
            يظهر هنا كل سيرفر لديك فيه صلاحية الإدارة (مالك أو Administrator) والبوت Vexon مضاف إليه.
            القائمة تُقرأ من ديسكورد مباشرة عبر توكن جلستك.
          </p>
          ...
          <DiscordLinkPanel
            ...
            onLink={() => {
              setBusy(true);
              void signIn("discord", { callbackURL: "/dashboard" }).catch(() => setBusy(false));
            }}
          />
          ...
          {statusErr && (
            <div className="mt-6 rounded-2xl border border-danger/30 bg-danger-soft p-4 text-sm text-danger">
              ...
              <button
                type="button"
                className="mt-3 rounded-lg bg-danger px-3 py-1.5 text-xs font-semibold text-white transition hover:opacity-90"
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-xs text-muted">
                      {g.memberCount.toLocaleString("en-US")} عضو
                      {g.owner ? " · مالك" : g.administrator ? " · مدير" : ""}
                    </p>
                  </div>
...
/**
 * The Discord session is the root of the whole trust chain: without a
 * Better-Auth session created by Discord, the site has no user access token to
 * call `/users/@me/guilds` or sign the later bot request.
 */
function DiscordLinkPanel({
    ...
    return (
      <div className="mt-6 rounded-2xl border border-danger/30 bg-danger-soft p-5">
        ...
        <p className="mt-1.5 text-sm leading-relaxed text-muted">
          أضف <Code>DISCORD_CLIENT_ID</Code> و <Code>DISCORD_CLIENT_SECRET</Code> إلى متغيرات البيئة، وسجّل رابط Better Auth{" "}
          <Code>/api/auth/oauth2/callback/discord</Code> في تطبيق ديسكورد. بدونهما لا يمكن التحقق من صلاحياتك الحقيقية.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="mt-6 flex flex-col gap-4 rounded-2xl border border-border bg-card p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <p className="font-semibold">سجّل الدخول عبر ديسكورد</p>
          <p className="mt-1 max-w-xl text-sm leading-relaxed text-muted">
            يجب أن تكون جلسة Better Auth منشأة عبر Discord حتى نقرأ سيرفراتك وصلاحياتك مباشرة من Discord. لا يظهر أي سيرفر قبل ذلك.
          </p>
        </div>
        <button
          type="button"
        ...
        >
          ...
          {busy ? "جارٍ تسجيل الدخول…" : "المتابعة عبر Discord"}
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="mt-6 flex items-center justify-between gap-3 rounded-2xl border border-border bg-card px-4 py-3">
      ...
      <span className="shrink-0 text-xs text-muted">جلسة Discord مباشرة</span>
    </div>
  );
}
